...
<body>   
 ...
 <div class="main_container">   
  ...
  <!--retrieve past appointment-->
                ...
                FROM appointment a
                WHERE YEAR(a.appointmentDate) = YEAR(CURDATE()) - 1 ) GROUP BY hr.patientTRID
                UNION
                SELECT p.patientTRID, p.patientName , p.patientSurname,0 AS pastAppointmentNumber
                FROM patient p, hospitalrelation hr, appointment a, admin ca,doctor d
                WHERE p.patientTRID=hr.patientTRID AND ca.clinicName=d.clinicName
                AND ca.adminTRID=:adminTRID AND p.patientTRID NOT IN (SELECT p.patientTRID 
                FROM patient p, appointment a, hospitalrelation hr WHERE hr.appointmentID=a.appointmentID 
                AND p.patientTRID=hr.patientTRID and a.appointmentID IN (SELECT a.appointmentID
                FROM appointment a
